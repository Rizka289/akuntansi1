...
{   
    ...
    function karyawan_post(){
        ...
        $data = array(
            ...
            'status_perkawinan' => $post['sts'],
            'jml_anak' => $post['jml_anak'],
            'pendidikan' => $post['pendidikan'],
            'jabatan' => $post['jabatan'],
            'agama' => $post['agama'],
            'bank' => $post['bank'],
            'no_rek' => $post['norek'],
            'upah_pokok' => $post['upah'],
            // 'deskripsi' => !empty($post['desc']) ? $post['desc'] : null
        );
        try {
          if(!$isEdit){
                $this->db->insert('karyawan', $data);
                $new_data = $this->db->order_by('id', 'DESC')->get('karyawan')->row();
                $new_data->message = "Berhasil menambahkan karyawan baru";
                response($new_data);
          }else{
                $this->db->where('id', $post['id'])->update('karyawan', ['nama' => $post['nama'], 'nik' =>$post['nik'],
                'nrp'=> $post['nrp'],'no_hp'=> $post['nope'], 'jenis_kelamin'=>$post['jk']]);
